...
>
  ...
  <xsl:template name="toPascalCase">
    ...
    <xsl:if test="$value != ''">
      ...
      <xsl:choose>
        <xsl:when test="$segment != ''">
          <xsl:value-of
	      select="translate(substring($segment,1,1),$alpha,$ALPHA)"/><xsl:value-of
	      select="substring($segment,2)"/><xsl:if test="$keepDelimiter"><xsl:value-of select="$delimiter"/></xsl:if>
          <xsl:call-template name="toPascalCase">
            <xsl:with-param name="value" select="substring-after($value, $delimiter)"/>
            <xsl:with-param name="delimiter" select="$delimiter"/>
            <xsl:with-param name="keepDelimiter" select="$keepDelimiter"/>
  ...
  <xsl:template name="toStrictPascalCase">
    <xsl:param name="value"/>
    <xsl:param name="delimiter" select="'_'"/>
    <xsl:param name="keepDelimiter" select="false()"/>
    <xsl:if test="$value != ''">
      ...
      <xsl:choose>
        <xsl:when test="$segment != ''">
          <xsl:value-of
	      select="translate(substring($segment,1,1),$alpha,$ALPHA)"/><xsl:value-of
	      select="translate(substring($segment,2),$ALPHA,$alpha)"/><xsl:if test="$keepDelimiter"><xsl:value-of select="$delimiter"/></xsl:if>
          <xsl:call-template name="toStrictPascalCase">
            <xsl:with-param name="value" select="substring-after($value, $delimiter)"/>
            <xsl:with-param name="delimiter" select="$delimiter"/>
            <xsl:with-param name="keepDelimiter" select="$keepDelimiter"/>
          </xsl:call-template>    
        ...
        <xsl:otherwise>
          <xsl:value-of select="translate(substring($value,1,1),$alpha,$ALPHA)"/><xsl:value-of select="translate(substring($value,2),$ALPHA,$alpha)"/>
        </xsl:otherwise>
      </xsl:choose>      
    </xsl:if>
  </xsl:template>
    ...
    <xsl:template name="strictPascal">
    <xsl:param name="value" select="name"/>
    <xsl:param name="delimiter" select="'_'"/>
    <xsl:call-template name="escapeKeyword">
      <xsl:with-param name="value"><xsl:choose>
      <xsl:when test="$optionFixCase"><xsl:variable name="dotted"><xsl:call-template name="toStrictPascalCase">
          <xsl:with-param name="value" select="$value"/>
          <xsl:with-param name="delimiter" select="'.'"/>
          <xsl:with-param name="keepDelimiter" select="true()"/>
        </xsl:call-template></xsl:variable><xsl:call-template name="toStrictPascalCase">
          <xsl:with-param name="value" select="$dotted"/>
          <xsl:with-param name="delimiter" select="$delimiter"/>
        </xsl:call-template></xsl:when>
      <xsl:otherwise><xsl:value-of select="$value"/></xsl:otherwise>
