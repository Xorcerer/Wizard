...
// Note: requires additional types generated from: <xsl:value-of select="."/></xsl:template>
  ...
  --><xsl:if test="$optionPreObservable">, global::System.ComponentModel.INotifyPropertyChanging</xsl:if>
  {
    ...
    private global::ProtoBuf.IExtension _extensionObject;
    global::ProtoBuf.IExtension global::ProtoBuf.IExtensible.GetExtensionObject(bool createIfMissing)
      { return global::ProtoBuf.Extensible.GetExtensionObject(ref _extensionObject, createIfMissing); }
  }
  </xsl:template>
  ...
  <xsl:template match="EnumValueDescriptorProto">
      ...
      --><xsl:text disable-output-escaping="yes">
      </xsl:text><xsl:call-template name="strictPascal"/><xsl:text xml:space="preserve"> = </xsl:text><xsl:value-of select="$value"/><xsl:if test="position()!=last()">,
      </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="FieldDescriptorProto[default_value]" mode="defaultValue">
    <xsl:choose>
      ...
      <xsl:when test="type='TYPE_ENUM'"><xsl:apply-templates select="." mode="type"/>.<xsl:call-template name="strictPascal">
  ...
  <xsl:template name="GetFirstEnumValue">
	...
	<xsl:call-template name="strictPascal">
	  <xsl:with-param name="value" select="(value/EnumValueDescriptorProto)[1]/name"/>
	</xsl:call-template>
      </xsl:if>
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:template match="FieldDescriptorProto[label='LABEL_OPTIONAL' or not(label)]">
    ...
    [global::System.Runtime.Serialization.DataMember(Name=@"<xsl:value-of select="name"/>", Order = <xsl:value-of select="number"/>, IsRequired = false)]</xsl:if><xsl:call-template name="WriteGetSet">
